	...
	public GTHistory getHistory(File f) throws Exception{
		...
		try(MRepositoryManager repManager = MRepositoryManager.getManager(f);){
			...
			for (RevCommit revCommit : logCommand.call()) {
				// get data
				String sId = revCommit.getId().getName();
				...
				// TODO check if the modification is ok
				...
				commitDate.setTimeInMillis(revCommit.getCommitTime()*1000);
				commitDate.setTime(revCommit.getCommitterIdent().getWhen()); // XXX Added
				// set data
