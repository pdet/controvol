...
public class ControVolNature implements IProjectNature {
	...
	public void configure() throws CoreException {
		...
		setup.generateProperties(setup.getControVolFolder());
	...
	private void addBuilder() {
		if (!hasControVolBuilder()) {
			try {
				...
				buildSpecCmd.setBuilderName(Cst.BUILDER_ID);
		...
		if (hasControVolBuilder()) {
			try {
				...
				for (Integer i = 0 ; i < buildSpecCmds.length ; i++){
					if (!buildSpecCmds[i].getBuilderName().equals(Cst.BUILDER_ID)){
	...
	private void addNature() {
		if (!hasControVolNature()) {
			try {
				...
				newNatureIds[natureIds.length] = Cst.NATURE_ID;
		...
		if (hasControVolNature()) {
			try {
				...
				for (Integer i = 0 ; i < natureIds.length ; i++){
					if (!natureIds[i].equals(Cst.NATURE_ID)){
	...
	private Boolean hasControVolBuilder(){
		...
		try {
			...
			while (i < commands.length  && !found) {
				found = commands[i].getBuilderName().equals(Cst.BUILDER_ID);
				i++;
			}
		} catch (CoreException e) {
			e.printStackTrace();
	...
	private Boolean hasControVolNature(){
		...
		try {
			...
			while (i < natureIds.length  && !found) {
				found = natureIds[i].equals(Cst.NATURE_ID);
				i++;
			}
		} catch (CoreException e) {
			e.printStackTrace();
