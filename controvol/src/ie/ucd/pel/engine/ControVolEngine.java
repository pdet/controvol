...
public class ControVolEngine {
	...
	public Evolution getEvolution(List<MApplication> apps){
		...
		for (int i = apps.size()-1 ; i > 0 ; i--){
			evolutionAux = this.getEvolution(apps.get(i), apps.get(i-1));
		...
		for (Operation addAux : adds){
			...
			for (Operation delAux : dels){
				...
				if (cond1 && cond2 && cond3){
					errors.add(new AttributeCleaningNeeded((AddAttribute) addAux)); // TODO complete
					/*System.err.println("WARNING: Data cleaning recommended: the attribute '"+
							addAttributeName+"' of entity '"+delEntityName+"' was deleted (version '"+del.getVersion2()+"') "+
							"and then added again (version '"+add.getVersion2()+"'); "+
							"there is a risk of unexpected values.");*/
				}
